function view_agenda(retornos){
    ...
    if(retornos['murais']){ 
        for(i  in retorno){
            ...
            conteudo_lista += "<ons-card "+(retorno[i]['url_destino'] ? "class='clicavelagenda' valor='"+(retorno[i]['url_destino'])+"'" :"class='imagemview' imagem='"+(retorno[i]['arquivo'])+"'")+"><font class='font_tam1'>"+(retorno[i]['data_referente'])+" - "+(retorno[i]['assunto'])+"</font>";
